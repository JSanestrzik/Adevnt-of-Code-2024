...
void read_input_from_file(std::ifstream &f_stream, std::vector<long> &lhs, std::vector<long> &rhs) {
    std::string line;
    size_t pos = 0;
    ...
    while (std::getline(f_stream, line)) {
        while((pos = line.find(delim)) != std::string::npos) {
            lhs.emplace_back(std::stol(line.substr(0, pos)));
            line.erase(line.begin(), line.begin() + pos + delim.size());
            rhs.emplace_back(std::stol(line));
        }
    }
}
...
int main() {
    ...
    read_input_from_file(Input, left_list, right_list);
